...
async function loginComGoogle() {
  ...
  if (!snap.exists()) {
    ...
    // Captura indicador salvo no localStorage
    ...
    await setDoc(ref, {
      ...
      role: "user",
      criadoEm: Date.now(),
      indicadoPor: indicadoPor,        // SALVA CORRETAMENTE
      bonusRecebido: false
    });

    // SALVA também na coleção indicacoes
    if (indicadoPor) {
    ...
    // BONUS DE INDICAÇÃO (IMEDIATO APENAS SE QUISER)
    if (indicadoPor) {
      const refIndicador = doc(db, "users", indicadoPor);
  ...
  let roleFinal = data.role || "user";

  if (roleFinal !== "admin" && roleFinal !== "superadmin") {
    roleFinal = "user";
  }
